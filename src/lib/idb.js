...
export function idbGet(db, k, store = SAVE_STORE) { return new Promise((res, rej) => {
  const t = db.transaction(store, "readonly").objectStore(store).get(k);
  t.onsuccess = () => res(t.result); t.onerror = () => rej(t.error);
});}

export function idbPut(db, k, v, store = SAVE_STORE) { return new Promise((res, rej) => {
  const t = db.transaction(store, "readwrite").objectStore(store).put(v, k);
  t.onsuccess = () => res(); t.onerror = () => rej(t.error);
});}

export function idbDel(db, k, store = SAVE_STORE) { return new Promise((res, rej) => {
