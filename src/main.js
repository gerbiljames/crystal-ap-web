...
async function bootEmulatorAndUi() {
  ...
  const landscapeMQ = window.matchMedia("(max-width: 900px) and (orientation: landscape)");
  const relocateLogArea = () => {
    if (!logAreaEl || !playGameEl || !playControlsEl || !gamepadEl) return;
    // Mobile portrait: slot between emulator and gamepad so it fills
    // the slack. Mobile landscape + desktop: keep in play-controls so
    // it flows below the play area.
  ...
  landscapeMQ.addEventListener("change", () => { relocateLogArea(); updateEmuMaxH(); });
  const doMeasure = () => {
    ...
    // Real chrome inside the screen-frame = padding + row gap + label
    // row + wrap padding. Computed from parts because the frame may be
    // stretched to fill a grid cell (landscape), in which case
    // frame.offsetHeight - canvas.offsetHeight reflects all the
    // stretched slack, not the actual chrome.
    ...
    // Landscape splits the gamepad around the emulator via display:
    // contents, so only the start/select row sits under the emulator.
    const landscape = window.matchMedia("(max-width: 900px) and (orientation: landscape)").matches;
    let budget;
    if (landscape) {
      // Emulator spans the full play-game height; A/B + start/select
      // stack in the right column and don't eat vertical space.
      // Extra 8px safety margin for rendering/sub-pixel slack.
  ...
  // --- on-screen gamepad ---
  // Pointer Events unify touch + mouse, and releasing pointer capture
  // on pointerdown means subsequent pointermove events report the
  // element currently under the finger/cursor (not the original
  // pointerdown target). That's what lets a finger roll from one gp-btn
  // to another without lifting.
  ...
  // Touch events for mobile: track by identifier, re-bind via
  // elementFromPoint on every touchmove so rolling re-presses the
  // button currently under the finger.
  // Map value is the button currently held by that touch, or null if
  // the finger is between buttons. Keeping the entry (as null) lets a
  // touch that rolls off a button later roll back onto another one.
